package parse
{
   ...
   public class Lex extends EventDispatcher
   {
	  ...
	  public var isErr:Boolean = false;
         ...
         if(_str)
         {
            ...
            while(this.ptr < this.str.length + 1)
            {
               ...
               if (tk){
                  ...
                  this.words.push(tk);
				   if (words.length>=4&&words[words.length-1].type==TokenType.ident&&words[words.length-4].type==TokenType.ident&&words[words.length-2].type==TokenType.Colon&&words[words.length-3].type==TokenType.Colon){
					   var tk4:Token = words.pop();
					   var tk3:Token = words.pop();
					   var tk2:Token = words.pop();
					   var tk1:Token = words[words.length - 1];
					   tk1.word += "::" + tk4.word;
					   tk1.value = tk1.word;
					   tk1.type = TokenType.config;
				   }
               }
            }
         }
         ...
         switch(this.ch)
         {
            ...
            default:
               ...
               else if(this.isAlpha(this.ch))
               {
                  ...
                  if(Token.wordpatten.indexOf("|" + this.word + "|") >= 0)
                  {
                     if(this.word == "as" || this.word == "is" || this.word == "in" || this.word == "instanceof")
                     {
                        token.type = TokenType.COP;
                        token.word = this.word;
                     }else if (this.word=="const"){
						token.type = TokenType.keyvar;
						token.word = this.word;
					 }
                     else
                     {
               ...
               else
               {
				  ...
				  trace("lex" + ".as:" + lines[line] + ": Error: 词法分析错误" + this.str.substr(this.ptr, 1));
				  this.parseError(this.ptr + "Unexpected " + this.str.substr(this.ptr,10) + " encountered");
               }
         }
         return token;
      }
      ...
      {
		  isErr = true;
		  err=new Error(this.lines[this.line] + "词法分析出错:" + message + "当前位置=" + line+"行:"+this.ptr);
         throw err;
      }
   }
}
